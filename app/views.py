...
def customer_create():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            customer = models.Customer(name=request.form.get('name'), phone_number=request.form.get('phone_number'),
                                       item=request.form.get('item'), quantity=request.form.get('quantity'),
        ...
        if customer.user_id == current_user.id:
            ...
            if request.method == 'POST':
                if form.validate_on_submit():
                    ...
                    db.session.commit()
                    flash('Клиент обновлен', category='succes')
                    return redirect(url_for('index'))
                if form.errors:
                    for errors in form.errors.values():
                        for error in errors:
